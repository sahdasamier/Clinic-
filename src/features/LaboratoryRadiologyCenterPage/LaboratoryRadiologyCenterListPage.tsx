...
function TabPanel(props: TabPanelProps) {
  ...
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`laboratoryRadiology-tabpanel-${index}`}
      aria-labelledby={`laboratoryRadiology-tab-${index}`}
      {...other}
    >
...
const InventoryListPage: React.FC = () => {
  ...
  const laboratoryRadiologyItems = [
    { 
      id: 1, 
      name: 'Surgical Masks', 
      quantity: 500, 
  ...
  const lowStockItems = laboratoryRadiologyItems.filter(item => item.status === 'low-stock' || item.status === 'critical');
  const totalValue = laboratoryRadiologyItems.reduce((sum, item) => sum + (item.quantity * item.cost), 0);
            ...
            <Box sx={{ 
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', width: { xs: '100%', md: 'auto' } }}>
                ...
                <Box>
                  ...
                  >
                    {t('laboratoryRadiology_management')}
                  </Typography>
                  <Typography 
                    variant="h6"
                    sx={{ 
              ...
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3, textAlign: 'center' }}>
                  ...
                  <Typography variant="h4" sx={{ fontWeight: 800, color: 'text.primary', mb: 0.5 }}>
                    {laboratoryRadiologyItems.length}
                  </Typography>
          ...
          <Card sx={{ 
            ...
            <Box sx={{ p: 3, borderBottom: 1, borderColor: 'divider' }}>
              <Grid container spacing={2} alignItems="center">
                <Grid item xs={12} md={6}>
                  <TextField
                    fullWidth
                    placeholder={t('search_laboratoryRadiology_items')}
                    value={searchQuery}
            ...
            <Box sx={{ borderBottom: 1, borderColor: 'divider', px: 3 }}>
              <Tabs value={tabValue} onChange={handleTabChange}>
                <Tab label={`${t('all_items')} (${laboratoryRadiologyItems.length})`} />
                <Tab 
                  label={
            ...
            <TabPanel value={tabValue} index={0}>
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {laboratoryRadiologyItems.map((item) => (
              ...
              <Box sx={{ p: 3 }}>
                ...
                <Grid container spacing={3}>
                  {['PPE', 'Medical Devices', 'Pharmaceuticals'].map((category) => {
                    const categoryItems = laboratoryRadiologyItems.filter(item => item.category === category);
                    return (
