...
const Sidebar: React.FC = () => {
  ...
  // ✅ Add focus management effect to prevent aria-hidden accessibility warnings
  React.useEffect(() => {
    // When sidebar state changes, clear any focused elements to prevent conflicts
    if (isMobile && isCollapsed) {
      const activeElement = document.activeElement as HTMLElement;
      if (activeElement && activeElement.blur && 
          activeElement.closest('.MuiDrawer-paper')) {
        activeElement.blur();
      }
    }
    ...
    return (
      <>
        <SwipeableDrawer
          ...
          // ✅ Handle focus management on close to fix accessibility warning
          onClose={(event) => {
            // Clear any focused elements before closing to prevent aria-hidden focus conflict
            const activeElement = document.activeElement as HTMLElement;
            if (activeElement && activeElement.blur) {
              activeElement.blur();
            }
            // Small delay to ensure focus is cleared before aria-hidden is applied
            setTimeout(() => {
              toggleSidebar();
            }, 10);
          }}
          ModalProps={{
            ...
            // ✅ Additional focus management for backdrop clicks - Fixed deprecation warning
            onClose: (event, reason) => {
              // Remove focus from any element inside the drawer before closing
              const activeElement = document.activeElement as HTMLElement;
              if (activeElement && activeElement.blur) {
                activeElement.blur();
              }
              ...
              if (reason === 'backdropClick') {
                // Clear focus and close the drawer
                const activeElement = document.activeElement as HTMLElement;
                if (activeElement && activeElement.blur) {
                  activeElement.blur();
                }
