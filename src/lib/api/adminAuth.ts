...
import { getOptimizedAuth, getOptimizedFirestore, firebaseManager } from './../firebase/legacy-compat';
...
const createSecondaryApp = async (): Promise<{ app: FirebaseApp; auth: FirebaseAuth }> => {
  ...
  const config = await import('./../firebase/legacy-compat').then(m => m.validateFirebaseConfig());
